...
---
title: "Vignette: Synthetic Examples for Multi-Path AIC"
author: "Nimra Ismail"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Synthetic Examples for Multi-Path AIC}
...
Initially, we show base R–style demonstrations (Sections 5.1 and 5.2) to confirm that the AIC-based model comparisons are functioning properly. Then, in Section 5.3, we demonstrate how to use the actual package workflow (`multi_path_AIC_procedure`) to perform a full end-to-end analysis.
...
The purpose of this section is to replace the original placeholder example from the assignment (which used `build_paths()`, `stability()`, and `plausible_models ()`) with a single, unified call to your implemented function `multi_path_AIC_procedure()`.

We show both linear and logistic regression examples and visualize the stability of variable selection using `plot_stability()`. This section confirms that the package functions integrate properly and produce interpretable, stable results for both regression families.
...
res_linear <- multi_path_AIC_procedure(
  data = df,
  response = "y",
  K = 5,                # number of forward-selection steps
  epsilon = 1e-6,       # convergence tolerance
  delta = 2,            # AIC tolerance for path branching
  L = 25,               # number of paths explored
  B = 50,               # bootstrap resamples for stability
  delta_AIC = 2,        # threshold for plausible models
  tau = 0.6,            # stability threshold
  model_type = "linear" # specify model family
)
