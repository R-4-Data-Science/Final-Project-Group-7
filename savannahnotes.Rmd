...
build_paths <- function(X, response, K = 5, epsilon = 10, delta = 200, L = 25, model_type = NULL) {
  ...
  # --- Auto-detect model type ---
  detect_model_type <- function(X[[response]]) {
    if (is.factor(response) || length(unique(response)) == 2) return("logistic")
    if (is.numeric(response)) return("linear")
    stop("Cannot detect model type automatically.")
  }
  ...
  predictors <- colnames(X)
  predictors <- predictors[predictors != response]  # exclude response
