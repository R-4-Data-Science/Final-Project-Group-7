  ...
  k <- 1
    ...
    for (i in 1:K) {
      ...
      if(k == 1){
        ...
        start_aic[i] <- summary(fit[[i]])
        
        start_models[[i]] <- list(variables = predictors[i], fit = fit[[i]], AIC = )
...
build_paths(X = adult[,1:14], response = adult[,15], K = 5, epsilon = 10, delta = 200, L = 25,
                        model_type = NULL)
