...
test <- build_paths(data = adult[,1:14], response = adult[,15])
...
Section 3.1
```{r}
test <- build_paths(adult[,1:14], response = adult[,15])
...
build_paths <- function(X, response, K = 5, epsilon = 10, delta = 200, L = 25,
  ...
  # *** input vars is the colnames of variables? **** yes I think so
  fit_model <- function(vars) {
...
Version 1 for Part 3 Plausible Models
```{r}
